...
export default function OrganizerDashboard() {
  const { data: stats, isLoading: statsLoading } = useFetch<any>("/api/organizer/stats");
  const { data: recentEvents, isLoading: eventsLoading } = useFetch<Array<any>>("/api/organizer/events/recent");
        ...
        <header className="h-16 px-4 md:px-8 border-b border-white/10 bg-card/30 backdrop-blur-xl flex items-center justify-between z-10 sticky top-0">
          ...
          <div className="flex items-center gap-4 flex-1 justify-end max-w-2xl">
            ...
            <Avatar className="h-8 w-8 sm:hidden shrink-0">
              <AvatarFallback className="bg-primary/20 text-primary">{(stats?.organizerInitials) ?? 'O'}</AvatarFallback>
            </Avatar>
          </div>
        </header>
          ...
          <div className="max-w-6xl mx-auto space-y-8">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
              {statsLoading ? (
                ...
                [
                  { label: "Total Events", value: stats?.totalEvents ?? '—', change: stats?.eventsChange ?? '' , icon: CalendarDays, color: "text-purple-400", bg: "bg-purple-500/10" },
                  { label: "Total Revenue", value: stats?.totalRevenue ? `₹${stats.totalRevenue.toLocaleString('en-IN')}` : '—', change: stats?.revenueChange ?? '', icon: TrendingUp, color: "text-green-400", bg: "bg-green-500/10" },
                  { label: "Tickets Sold", value: stats?.ticketsSold ?? '—', change: stats?.ticketsChange ?? '', icon: CreditCard, color: "text-blue-400", bg: "bg-blue-500/10" },
                  { label: "Total Attendees", value: stats?.attendees ?? '—', change: stats?.attendeesChange ?? '', icon: Users, color: "text-orange-400", bg: "bg-orange-500/10" },
                ].map((stat, i) => (
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <CardContent>
                  <div className="h-[300px] w-full">
                    <ResponsiveContainer width="100%" height="100%">
                      <AreaChart data={stats?.revenueSeries || []} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
                        <defs>
              ...
              <Card className="glass-panel border-white/5">
                ...
                <CardContent>
                  <div className="h-[300px] w-full">
                    <ResponsiveContainer width="100%" height="100%">
                      <BarChart data={stats?.attendanceSeries || []} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
                        <CartesianGrid strokeDasharray="3 3" stroke="#ffffff10" vertical={false} />
                        <XAxis dataKey="name" stroke="#ffffff50" fontSize={12} tickLine={false} axisLine={false} />
                        ...
                        <Bar dataKey="value" radius={[4, 4, 0, 0]}>
                          {(stats?.attendanceSeries || []).map((entry: any, index: number) => (
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                    ...
                    <ResponsiveContainer width="100%" height="100%">
                      <PieChart>
                        <Pie
                          ...
                          stroke="none"
                        >
                          {(stats?.popularEvents || []).map((entry: any, index: number) => (
              ...
              <Card className="glass-panel border-white/5 lg:col-span-2">
                ...
                <CardContent className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                  ...
                  {/* Analytics quick-action removed per organizer UX simplification */}
                </CardContent>
              </Card>
